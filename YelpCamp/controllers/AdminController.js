...
const { templateForgetPassword } = require('../templateMail/templateForgetPassword')
...
module.exports.login = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            ...
            const token = jwt.sign(userInfo, _CONF.SECRET, { expiresIn: _CONF.tokenLife });
            const refreshToken = jwt.sign(userInfo, _CONF.SECRET_REFRESH, { expiresIn: _CONF.refreshTokenLife});
        ...
        if (code === _CONF.GENERATE.code) {
            bcrypt.genSalt(10, async function(err, saltRounds) {
                ...
                bcrypt.hash(passwordNew, saltRounds, async function(err, hash) {
                    ...
                    const user = await adminModel.findOneAndUpdate({ email }, { password: hash, saltRounds: saltRounds });
                    if (!user) {
                        return res.status(404).json({ message: "User not found" });
                    }
